    ...
    public class NewCharacterConnectionContext : ConnectionContextBase
    {
        ...
        async Task Complete()
        {
            using (var userRepo = Services.GetService<IUserRepository>())
            {
                ...
                _user.PasswordSalt = hasher.GenerateSalt();
                _user.Password = hasher.ComputeHash(_password.ToUnsecureString(), _user.PasswordSalt);
                _user.IsActive = true;

                await userRepo.SaveUser(_user);
            }
            using (var charRepo = Services.GetService<IObjectRepository>())
            {
                await charRepo.SaveObject(new PlayerCharacter
                {
                    UserId = _user.Id,
                    Name = _user.Name
                });
